...
from users.models import DateTimeSlot, TimeSlot, UserProfile
from users.models import Review
from users.serializers import AccessRequestSerializer, DateTimeSlotSerializer, TimeSlotSerializer
    ...
    def get_dashborad_table(self, request):
        ...
        if(table_type == "appointment"):
            appointments = AppointmentModel.objects.filter(
                patient=user).order_by("-id")
            appointments_data = AppoitmentSerializer(
                appointments, many=True).data

            return Response({"table_rows": appointments_data})

        elif table_type == 'prescription':
            prescriptions = Prescription.objects.filter(
                customer=user).order_by("-id")

            prescriptions_data = PrescriptionSerializer(
                prescriptions, many=True).data

            return Response({"table_rows": prescriptions_data})

        elif table_type == 'billing':
            billings = BillingInvoice.objects.filter(
                customer=user).order_by("-id")

            billings_data = BillingInvoiceSerializer(
                billings, many=True, context={"request": request}).data
    ...
    def create_invoice(self, request, *args, **kwargs):
        ...
        if True:
            ...
            invoice_data = BillingInvoiceSerializer(
                billing_invoice, many=False, context={"request": request}).data
